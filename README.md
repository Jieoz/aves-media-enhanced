...
- Mounted-volume validation before treating an inaccessible path as deleted
- Scoped-storage move detection via `RELATIVE_PATH` + `DISPLAY_NAME` instead of `DATA` on Android 10+
...
The signing key is stored only as encrypted GitHub Actions secrets. CI uses the repository-pinned Flutter SDK and Gradle wrapper, runs Flutter and Android unit tests, and verifies the exact signed split APKs before upload. A release is published only after those artifacts are downloaded and their package, version, signer, architecture, integrity, and hashes are verified again.
